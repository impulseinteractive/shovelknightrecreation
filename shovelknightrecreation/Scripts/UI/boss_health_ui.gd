...
func _ready() -> void:
	...
	print("[BOSS UI] boss max health: ", ui_max_health)
	...
	print("[BOSS UI] life container count: ", life_bar_container.get_child_count())
	return
...
func _process(delta: float) -> void:
	if debug_input and Input.is_action_just_pressed("ui_page_up"):
		ui_current_health -= 1
		boss_ref.on_health_changed.emit(ui_current_health)
		pass
	pass
